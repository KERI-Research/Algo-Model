	...
	return (
		<>
			...
			<section className="card" aria-labelledby="model-heading">
				...
				<DefinitionList
					items={[
						...
						["Deployment", model.deployment_version],
						["Trained", model.created_at],
						["Inference backend", `${model.inference_backend} (exported weights)`],
						["Training rows", (model.training_rows || 0).toLocaleString("en-GB")],
						[
							"Reference rows scored",
							(model.reference_rows_scored || 0).toLocaleString("en-GB"),
						],
			...
			<div className="grid grid-2">
				<section className="card" aria-labelledby="architecture-heading">
					<h2 id="architecture-heading">Architecture</h2>
					<DefinitionList
						items={[
							["Type", architecture.type],
							["Input features", architecture.input_features],
							["Encoded dimension", architecture.transformed_dimension],
							["Latent dimension", architecture.latent_dimension],
							[
								"Hidden layers",
								Array.isArray(architecture.hidden_layers)
									? architecture.hidden_layers.join(" - ")
									: architecture.hidden_layers,
							],
							["Activation", architecture.activation],
							["Objective", architecture.objective],
							["Deviation score", architecture.score_definition],
						]}
					/>
				...
				<section className="card" aria-labelledby="claims-heading">
					<h2 id="claims-heading">What this deployment reports</h2>
					<h4>Supported outputs</h4>
					<ul>
						{model.supported_outputs.map((item) => (
							<li key={item}>
								<code>{item}</code>
							</li>
						))}
					...
					<ul>
						{model.prohibited_outputs.map((item) => (
							<li key={item}>{item}</li>
						))}
					</ul>
				</section>
			...
			<section className="card" aria-labelledby="limits-heading">
				<h2 id="limits-heading">Current data limitations</h2>
				{data.data_limitations.length === 0 ? (
					...
					<div className="table-wrap">
						<table>
							<caption>
								These constraints set the ceiling on every claim below.
							</caption>
							<thead>
								<tr>
							...
							<tbody>
								{data.data_limitations.map((item) => (
									<tr key={item.id}>
										...
										<td className="wrap">{item.detail}</td>
									</tr>
								))}
							</tbody>
						</table>
			...
			<Notice kind="caution" title="Clinician review required.">
